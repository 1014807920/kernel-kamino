...
static int rt_power_probe(struct platform_device *pdev)
{
	...
	pi->ac_psy->desc.get_property = rtpower_get_property;
	pi->ac_psy->desc.set_property = rtpower_set_property;
	ret = power_supply_register(&pdev->dev, &pi->ac_psy->desc);
	if (ret < 0) {
	...
	pi->usb_psy->desc.get_property = rtpower_get_property;
	pi->usb_psy->desc.set_property = rtpower_set_property;
	ret = power_supply_register(&pdev->dev, &pi->usb_psy->desc);
	if (ret < 0) {
