	...
	for (i = 0; i < VCC_152_MAX; i++) {
		...
		if (info->desc.id == AXP152_ID_LDO4
			...
			regu_data->regulators[i] =
			    axp_regulator_register(&pdev->dev, axp_dev->regmap,
						   &axp_regl_init_data[i],
						   info);
			...
			regu_data->regulators[i] =
			    axp_regulator_sel_register(&pdev->dev,
						       axp_dev->regmap,
						       &axp_regl_init_data[i],
						       info);
		}
		...
		if (IS_ERR(regu_data->regulators[i])) {
			dev_err(&pdev->dev,
				...
				info->desc.name);
			while (--i >= 0)
				axp_regulator_unregister(regu_data->
							 regulators[i]);
...
static const struct of_device_id axp152_regu_dt_ids[] = {
	{.compatible = "axp, axp152-regulator",},
	{},
};
