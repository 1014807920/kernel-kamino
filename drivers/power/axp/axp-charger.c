...
#include "axp2585/axp2585.h"
#include "axp2585/axp2585-charger.h"
#include "../../../kernel/power/power.h"

#define AXP2585_LOCK "axp2585"
				...
				struct axp_adc_res *adc)
{
	...
	axp_regmap_reads(map, chg_dev->spy_info->batt->bat_temp_offset, 2, tmp);
...
{
	if (!
	    (val & ((1 << batt->acpresent_bit) | (1 << batt->vbuspresent_bit))))
		return 1;
	else
		return 0;
}
...
void axp_charger_update_state(struct axp_charger_dev *chg_dev)
{
	...
	axp_regmap_reads(map, AXP2585_RDC0, 2, valtmp);
	tmp = ((valtmp[0] & 0x1F) << 8) + valtmp[1];
	chg_dev->rdc_value = ((tmp * 10742) - 5371) / 10000;

	axp_regmap_read(map, AXP2585_COULOMB_CTL, &chg_dev->reg_b8);
...
static void axp_usb(struct work_struct *work)
{
	...
	if (axp_usb_connect == 1) {
		...
		AXP_DEBUG(AXP_CHG, chg_dev->chip->pmu_num,
			  ...
			  usb_conn_state);
		if (usb_conn_state == 1) {	// 1 : usb 0: others
			axp_usbcur(CHARGE_USB_20);
			axp_usbvol(CHARGE_USB_20);

			usb->set_usb_ihold(chg_dev,
					   axp2585_config.
					   pmu_SDP_vbus_currentlimit);
			batt->set_chg_cur(chg_dev,
					  axp2585_config.
					  pmu_SDP_Cbat_currentlimit);
			chg_dev->usb_det = 1;
			usb_recheck = 0;
			...
			if (usb_recheck++ != 1) {
				...
				//  if(chg_dev->bc_result == BC_DCP) {
				axp_usbcur(CHARGE_AC);
				axp_usbvol(CHARGE_AC);
				usb->set_usb_ihold(chg_dev,
						   axp2585_config.
						   pmu_DCP_vbus_currentlimit);
				batt->set_chg_cur(chg_dev,
						  axp2585_config.
						  pmu_DCP_Cbat_currentlimit);
				chg_dev->ac_det = 1;
				chg_dev->usb_det = 0;
				usb_recheck = 0;
				//  }
	...
	AXP_DEBUG(AXP_CHG, chg_dev->chip->pmu_num,
		  "[axp_usb] axp_usbcurflag(0:ac,1:usb) = %d\n",
		  axp_usbcurflag);
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_MODEL_NAME:
		...
		break;
	case POWER_SUPPLY_PROP_PRESENT:
		val->intval = chg_dev->usb_det;
		break;
	case POWER_SUPPLY_PROP_ONLINE:
		if (chg_dev->ac_det)
			val->intval = 2;
		else if (chg_dev->usb_det)
			val->intval = 1;
		else
			val->intval = 0;
		break;
...
void axp_usbac_in(struct axp_charger_dev *chg_dev, int chargerinf)
{
	...
	axp_usbcur(chargerinf);
	axp_usbvol(chargerinf);

	AXP_DEBUG(AXP_CHG, chg_dev->chip->pmu_num, "axp ac/usb in : %d\n",
		  chargerinf);
	 ...
	 */
	axp_charger_update_state(chg_dev);
	mod_timer(&chg_dev->usb_status_timer, jiffies + msecs_to_jiffies(1000));
	axp_usb_ac_check_status(chg_dev);
}
...
irqreturn_t axp_usb_in_isr(int irq, void *data)
{
	...
	chg_dev->usb_det = 1;
	chg_dev->ac_det = 0;
	axp_usbac_in(chg_dev, CHARGE_USB_20);

	pm_wake_lock(AXP2585_LOCK);

	return IRQ_HANDLED;

}

irqreturn_t axp_bc_detect_isr(int irq, void *data)
{
	struct axp_charger_dev *chg_dev = data;
	//u8 bc_result = 0;
	//struct axp_regmap *map = chg_dev->chip->regmap;

	AXP_DEBUG(AXP_INT, chg_dev->chip->pmu_num, "Entering isr :%s\n",
		  __func__);

	// axp_regmap_read(map, AXP2585_STATUS2, &bc_result);
//    chg_dev->bc_result = (bc_result & 0xe0) >> 5;
	return IRQ_HANDLED;
...
irqreturn_t axp_ac_in_isr(int irq, void *data)
{
	...
	axp_change(chg_dev);
	axp_usbac_in(chg_dev, CHARGE_AC);
						  ...
						  struct axp_supply_info *info)
{
	...
	axp_usb_connect = 1;
	/* detect for state.online */
	axp_regmap_read(chg_dev->chip->regmap,
			chg_dev->spy_info->usb->det_offset, &val);
	chg_dev->usb_det = (val & 1 << chg_dev->spy_info->usb->det_bit) ? 1 : 0;
	pr_err("\nlulu: chg_dev->usb_det = %d\n\n", chg_dev->usb_det);
	axp_usb_ac_check_status(chg_dev);
	axp_battery_update_vol(chg_dev);
			 ...
			 struct axp_config_info *axp_config)
{
	...
	AXP_OF_PROP_READ(pmu_SDP_vbus_currentlimit, 4);
	AXP_OF_PROP_READ(pmu_SDP_Cbat_currentlimit, 0x10);
	AXP_OF_PROP_READ(pmu_CDP_vbus_currentlimit, 4);
	AXP_OF_PROP_READ(pmu_CDP_Cbat_currentlimit, 0x10);
	AXP_OF_PROP_READ(pmu_DCP_vbus_currentlimit, 4);
	AXP_OF_PROP_READ(pmu_DCP_Cbat_currentlimit, 0x10);
