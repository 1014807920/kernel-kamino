...
static struct rt5036_charger_info *g_ci;
static int rt_charger_get_property(struct power_supply *psy,
				   enum power_supply_property psp,
				   union power_supply_propval *val)
{
	struct rt5036_charger_info *ci = g_ci;
	int ret = 0;
	int regval = 0;
	switch (psp) {
				   ...
				   const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (ci->online) {
			...
			if (psy) {
				...
				psy->desc->set_property(psy, POWER_SUPPLY_PROP_STATUS,
						  &pval);
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (val->intval < 0) {
			...
			pval.intval = 0;
			psy->desc->set_property(psy, POWER_SUPPLY_PROP_ONLINE, &pval);
			power_supply_changed(psy);
			ci->otg_en = 1;
			if (ci->charge_cable != POWER_SUPPLY_TYPE_MAINS) {
				/* otg drop fix */
				...
				rt5036_charger_desc.set_propert(&ci->psy,
					POWER_SUPPLY_PROP_VOLTAGE_NOW,
					&pval);
#ifdef CONFIG_CHARGER_RT5036_VMID_HDMI
			...
			rt5036_set_bits(ci->i2c, 0x23, 0x3);
			pval.intval = ci->otg_volt;
			ci->psy->desc->set_property(ci->psy,
					     POWER_SUPPLY_PROP_VOLTAGE_NOW,
					     &pval);
#ifdef CONFIG_CHARGER_RT5036_VMID_HDMI
			ret = rt5036_set_bits(ci->i2c, RT5036_REG_CHGCTL6, 0x20);
...
static const struct power_supply_desc rt5036_charger_desc = {
	.name			= "rt-charger",
	.type			= POWER_SUPPLY_TYPE_MAINS,
	.properties		= rt_charger_props,
	.num_properties		= ARRAY_SIZE(rt_charger_props),
	.get_property		= rt_charger_get_property,
	.set_property		= rt_charger_set_property,
...
#endif /* #ifdef CONFIG_CHARGER_RT5036_VMID_HDMI */
				pval.intval = 1;
				psy->desc->set_property(psy, POWER_SUPPLY_PROP_ONLINE,
						  &pval);
				power_supply_changed(psy);
			} else
...
static void rt5036_batabs_irq_handler(void *info, int eventno)
{
	...
	/*set aicr to 2000*/
	val.intval = 2000;
	ret = psy->desc->set_property(psy, POWER_SUPPLY_PROP_CURRENT_AVG, &val);
	if (ret < 0)
		dev_err(ci->dev, "set aicr to 2000 fail\n");
	/*set icc to 2000*/
	val.intval = 2000;
	ret = psy->desc->set_property(psy, POWER_SUPPLY_PROP_CURRENT_NOW, &val);
	if (ret < 0)
		dev_err(ci->dev, "set icc to 2000 fail\n");
	/*set charger offline*/
	val.intval = 0;
	ret = psy->desc->set_property(psy, POWER_SUPPLY_PROP_ONLINE, &val);
	if (ret < 0)
	...
	if (psy) {
		val.intval = 0;
		ret = psy->desc->set_property(psy, POWER_SUPPLY_PROP_PRESENT, &val);
		if (ret < 0)
	...
	if (RT5036_ACIN_LEVEL == gpio_get_value(ci->acdet_gpio)) {
		...
		if (psy) {
			...
			if (ci->otg_en) {
				/*set otg voltage*/
				pval.intval = ci->otg_volt;
				rt5036_charger_desc.set_property(&ci->psy,
					POWER_SUPPLY_PROP_VOLTAGE_NOW,
					&pval);
				rt5036_set_bits(ci->i2c, RT5036_REG_CHGCTL1,
						RT5036_CHGOPAMODE_MASK);
...
static int rt5036_charger_probe(struct platform_device *pdev)
{
...
#endif /* #ifdef CONFIG_RT_POWER */
	struct rt5036_charger_info *ci;
	struct power_supply_config rt_charger_cfg = {};
	...
	rt_charger_cfg.drv_data = ci;
        g_ci = ci;

	ci->psy = power_supply_register(ci->dev, &rt5036_charger_desc, NULL);
        if (IS_ERR(ci->psy)) {
		dev_err(ci->dev, "couldn't create power supply for rt-charger.\n");
		ret = PTR_ERR(ci->psy);
		goto out_dev;
	}
...
late_initcall(rt5036_charger_init);
