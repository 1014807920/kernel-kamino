...
static int led_draw(char *buf, int length)
{
    int i;
    for(i=0; i<length; i++){
        led_pwm_set(&g_priv->leds[i].cdev, buf[i]);
	//printk("%x",buf[i]);
    }
    //printk("\n");
    return length;
...
static int is_led_busy(void)
{
    return 0;
}

static int led_brightness_set(int brightness)
{
    int i;
    for(i=0; i<g_priv->num_leds; i++)
        led_pwm_set(&g_priv->leds[i].cdev, brightness);
    return 0;
}
...
static int led_pwm_probe(struct platform_device *pdev)
{
	...
	yodabase_led_draw = led_draw;
	yodabase_is_led_busy = is_led_busy;
	yodabase_led_brightness_set = led_brightness_set;
#if 0
