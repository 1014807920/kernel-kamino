 ...
 *
 ******************************************************/
static int i2c_write(struct aw9163 *aw9163, unsigned char addr,
		     unsigned int reg_data)
{
	...
	struct i2c_msg msgs[] = {
		{.addr = aw9163->i2c->addr,.flags = 0,.len = 3,.buf = wbuf,},
	};
	wbuf[0] = addr;
...
static int i2c_read(struct aw9163 *aw9163, unsigned char addr,
		    unsigned int *reg_data)
{
	int ret;
	...
	struct i2c_msg msgs[] = {
		{.addr = aw9163->i2c->addr,.flags = 0,.len = 1,.buf =
		 &addr,}, {.addr = aw9163->i2c->addr,.flags =
			   I2C_M_RD,.len = 2,.buf = rbuf,},
	};
...
static int aw9163_i2c_write(struct aw9163 *aw9163, unsigned char reg_addr,
			    unsigned int reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
...
static int aw9163_i2c_read(struct aw9163 *aw9163, unsigned char reg_addr,
			   unsigned int *reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
...
#if 0
static int aw9163_i2c_write_bits(struct aw9163 *aw9163, unsigned char reg_addr,
...
static int aw9163_i2c_writes(struct aw9163 *aw9163, unsigned char reg_addr,
...
static int aw9163_auto_cali(struct aw9163 *aw9163)
{
	...
	for (i = 0; i < 6; i++) {
		aw9163_i2c_read(aw9163, AW9163_REG_KDATA0, &reg_val);
		buf[i] = reg_val;
	}
	...
	if (aw9163->cali_cnt == 4) {
		for (i = 0; i < 6; i++) {
			if ((sen_num & (1 << i)) == 0) {	// sensor used
				...
				} else if ((aw9163->rawdata_sum[i] >> 2) >
					   CALI_RAW_MAX) {
					if ((i % 2)
					    && ((ofr_cfg[i >> 1] & 0xFF00) ==
						0x1F00)) {
				...
				if (aw9163->cali_num > 0) {
					if (cali_dir[i] !=
					    aw9163->old_cali_dir[i]) {
						cali_dir[i] = 0;
						ofr_cfg[i >> 1] =
						    aw9163->old_ofr_cfg[i >> 1];
					}
				}
			}
		}
		...
		for (i = 0; i < 3; i++) {
			aw9163_i2c_write(aw9163, AW9163_REG_OFR1 + i,
					 ofr_cfg[i]);
		}
		aw9163_i2c_read(aw9163, AW9163_REG_GCR, &reg_val);
		reg_val |= 0x0002;
...
} static void aw9163_normal_mode_init(struct aw9163 *aw9163)
{
	...
	aw9163_i2c_write(aw9163, AW9163_REG_THR0, AW9163_NORMAL_THR0);	// s1 press thred setting
	aw9163_i2c_write(aw9163, AW9163_REG_THR1, AW9163_NORMAL_THR1);	// s1 press thred setting
	aw9163_i2c_write(aw9163, AW9163_REG_THR2, AW9163_NORMAL_THR2);	// s1 press thred setting
	aw9163_i2c_write(aw9163, AW9163_REG_THR3, AW9163_NORMAL_THR3);	// s1 press thred setting
	aw9163_i2c_write(aw9163, AW9163_REG_THR4, AW9163_NORMAL_THR4);	// s2 press thred setting
	aw9163_i2c_write(aw9163, AW9163_REG_THR5, AW9163_NORMAL_THR5);	// s3 press thred setting
	...
	aw9163_i2c_write(aw9163, AW9163_REG_AKSR, 0x0038);	// aks
#ifndef AW_AUTO_CALI
	aw9163_i2c_write(aw9163, AW9163_REG_INTER, 0x0038);	// signel click interrupt
#else /*  */
	aw9163_i2c_write(aw9163, AW9163_REG_INTER, 0x0038);	// frame interrupt
#endif /*  */
	...
	aw9163_i2c_write(aw9163, AW9163_REG_GDCFGR, 0x0107);	// gesture key selecti
	aw9163_i2c_write(aw9163, AW9163_REG_GESTR1, 0x1230);
	aw9163_i2c_write(aw9163, AW9163_REG_GESTR2, 0x3210);
...
#ifndef AW_AUTO_CALI
	aw9163_i2c_write(aw9163, AW9163_REG_GIER, 0x0003);	// gesture and double click enable
#else /*  */
...
} static irqreturn_t aw9163_irq(int irq, void *data)
{
	...
	if (aw9163->cali_flag) {
		...
		if (aw9163->cali_flag == 0) {
			if (aw9163->cali_used) {
				aw9163_i2c_read(aw9163, AW9163_REG_GCR,
						&reg_val);
				reg_val &= 0xFFFD;
				aw9163_i2c_write(aw9163, AW9163_REG_GCR,
						 reg_val);
			}
			aw9163_i2c_write(aw9163, AW9163_REG_INTER,
					 AW9163_NORMAL_INTER);
			aw9163_i2c_write(aw9163, AW9163_REG_GIER,
					 AW9163_NORMAL_GIER);
			if (aw9163->cali_used) {
				aw9163_i2c_read(aw9163, AW9163_REG_GCR,
						&reg_val);
	...
	aw9163_i2c_read(aw9163, AW9163_REG_GISR, &reg_gisr);
	pr_info("%s: kisr=0x%04x, kst=0x%04x , gisr=0x%04x\n", __func__,
		reg_kisr, reg_kst, reg_gisr);
	if (reg_kisr & 0x0020) {
		if (reg_kst & 0x0020) {
		...
		if (reg_kst & 0x0010) {
			input_report_key(aw9163->input, KEY_HOMEPAGE, 1);
		} else {
			input_report_key(aw9163->input, KEY_HOMEPAGE, 0);
		}
		input_sync(aw9163->input);
	} else if (reg_kisr & 0x0008) {
		if (reg_kst & 0x0008) {
			input_report_key(aw9163->input, KEY_BACK, 1);
		} else {
			input_report_key(aw9163->input, KEY_BACK, 0);
		}
		input_sync(aw9163->input);
	}
	aw9163_interrupt_clear(aw9163);
...
int aw9163_read_chipid(struct aw9163 *aw9163)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		if (ret < 0) {
			dev_err(aw9163->dev,
				"%s: failed to read register AW9163_REG_RSTR: %d\n",
				__func__, ret);
		}
		switch (reg_val) {
		case 0xb223:
 ...
 *
 ******************************************************/
static ssize_t aw9163_i2c_reg_store(struct device *dev,
				    ...
				    const char *buf, size_t count)
{
	...
	if (2 == sscanf(buf, "%x %x", &databuf[0], &databuf[1])) {
		aw9163_i2c_write(aw9163, (unsigned char)databuf[0],
				 (unsigned int)databuf[1]);
	}
	return count;
}

static ssize_t aw9163_i2c_reg_show(struct device *dev,
...
static ssize_t aw9163_i2c_hwen_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct aw9163 *aw9163 = dev_get_drvdata(dev);
...
static ssize_t aw9163_i2c_hwen_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct aw9163 *aw9163 = dev_get_drvdata(dev);
	ssize_t len = 0;
...
static ssize_t aw9163_i2c_rawdata_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
...
static ssize_t aw9163_i2c_rawdata_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	struct aw9163 *aw9163 = dev_get_drvdata(dev);
	ssize_t len = 0;
...
static ssize_t aw9163_i2c_base_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
...
static ssize_t aw9163_i2c_base_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct aw9163 *aw9163 = dev_get_drvdata(dev);
	ssize_t len = 0;
...
static ssize_t aw9163_i2c_delta_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
...
static ssize_t aw9163_i2c_delta_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct aw9163 *aw9163 = dev_get_drvdata(dev);
	ssize_t len = 0;
...
static ssize_t aw9163_i2c_irqstate_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
...
static ssize_t aw9163_i2c_irqstate_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct aw9163 *aw9163 = dev_get_drvdata(dev);
...
static DEVICE_ATTR(reg, S_IWUSR | S_IRUGO, aw9163_i2c_reg_show,
		   aw9163_i2c_reg_store);
static DEVICE_ATTR(hwen, S_IWUSR | S_IRUGO, aw9163_i2c_hwen_show,
		   aw9163_i2c_hwen_store);
static DEVICE_ATTR(rawdata, S_IWUSR | S_IRUGO, aw9163_i2c_rawdata_show,
		   aw9163_i2c_rawdata_store);
static DEVICE_ATTR(base, S_IWUSR | S_IRUGO, aw9163_i2c_base_show,
		   aw9163_i2c_base_store);
static DEVICE_ATTR(delta, S_IWUSR | S_IRUGO, aw9163_i2c_delta_show,
		   aw9163_i2c_delta_store);
static DEVICE_ATTR(irqstate, S_IWUSR | S_IRUGO, aw9163_i2c_irqstate_show,
		   aw9163_i2c_irqstate_store);
static struct attribute *aw9163_attributes[] =
    { &dev_attr_reg.attr, &dev_attr_hwen.attr, &dev_attr_rawdata.attr,
	&dev_attr_base.attr, &dev_attr_delta.attr, &dev_attr_irqstate.attr, NULL
};

static struct attribute_group aw9163_attribute_group = {.attrs =
	    aw9163_attributes
};
...
static void aw9163_brightness_work(struct work_struct *work)
{
	struct aw9163 *aw9163 =
	    container_of(work, struct aw9163, brightness_work);
	unsigned int imax = 0;
	unsigned int reg_val;
	...
	if (aw9163->cdev.brightness) {
		...
		// LED Control
		aw9163_i2c_write(aw9163, AW9163_REG_CMDR, 0xBF00 | aw9163->cdev.brightness);	// CMDR-LED1~LED20 PWM=0xFF
	}
}
 ...
 *
 ******************************************************/
static ssize_t aw9163_reg_store(struct device *dev,
...
static ssize_t aw9163_reg_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct aw9163 *aw9163 = container_of(led_cdev, struct aw9163, cdev);
...
static ssize_t aw9163_hwen_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t count)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
...
static ssize_t aw9163_hwen_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct aw9163 *aw9163 = container_of(led_cdev, struct aw9163, cdev);
...
static ssize_t aw9163_blink_store(struct device *dev,
				  struct device_attribute *attr,
...
static ssize_t aw9163_blink_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
...
static DEVICE_ATTR(register, S_IWUSR | S_IRUGO, aw9163_reg_show,
		   aw9163_reg_store);
static DEVICE_ATTR(hw_en, S_IWUSR | S_IRUGO, aw9163_hwen_show,
		   aw9163_hwen_store);
static DEVICE_ATTR(blink, S_IWUSR | S_IRUGO, aw9163_blink_show,
		   aw9163_blink_store);
static struct attribute *aw9163_led_attributes[] =
    { &dev_attr_register.attr, &dev_attr_hw_en.attr, &dev_attr_blink.attr,
	NULL
};

static struct attribute_group aw9163_led_attribute_group = {.attrs =
	    aw9163_led_attributes
};
 ...
 *
 ******************************************************/
static int aw9163_parse_led_cdev(struct aw9163 *aw9163, struct device_node *np)
{
	struct device_node *temp;
	int ret = -1;
	for_each_child_of_node(np, temp) {
		ret =
		    of_property_read_string(temp, "aw9163,name",
					    &aw9163->cdev.name);
		if (ret < 0) {
			dev_err(aw9163->dev,
		...
		ret = of_property_read_u32(temp, "aw9163,imax", &aw9163->imax);
		if (ret < 0) {
			dev_err(aw9163->dev,
		...
		ret =
		    of_property_read_u32(temp, "aw9163,brightness",
		...
		ret =
		    of_property_read_u32(temp, "aw9163,max_brightness",
	...
	if (ret) {
		dev_err(aw9163->dev, "unable to register led ret=%d\n", ret);
		goto free_pdata;
	}
	ret =
	    sysfs_create_group(&aw9163->cdev.dev->kobj,
	...
	return 0;
free_class:led_classdev_unregister(&aw9163->cdev);
free_pdata:return ret;
}
 ...
 *
 ******************************************************/
static int aw9163_i2c_probe(struct i2c_client *i2c,
			    const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(aw9163->reset_gpio)) {
		ret =
		    devm_gpio_request_one(&i2c->dev, aw9163->reset_gpio,
	...
	if (gpio_is_valid(aw9163->irq_gpio)) {
		ret =
		    devm_gpio_request_one(&i2c->dev, aw9163->irq_gpio,
	...
	return 0;
err_led:sysfs_remove_group(&i2c->dev.kobj, &aw9163_attribute_group);
err_sysfs:input_unregister_device(input_dev);
exit_input_register_device_failed:input_free_device(input_dev);
exit_input_dev_alloc_failed: err_irq: err_id:if (gpio_is_valid
	    (aw9163->irq_gpio))
...
#endif /*  */
static const struct i2c_device_id aw9163_i2c_id[] = { {AW9163_I2C_NAME, 0}, {}
};
...
static struct i2c_driver aw9163_i2c_driver = {.driver = {.name =
							 AW9163_I2C_NAME,
							 .owner = THIS_MODULE,
							 .of_match_table =
							 of_match_ptr
    ...
    aw9163_i2c_probe,.remove = aw9163_i2c_remove,.id_table = aw9163_i2c_id,
};
