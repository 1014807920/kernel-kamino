...
static struct pin_config {
    ...
    { "nationalchip,pin-val",PINCFG_TYPE_DAT },
    { "nationalchip,pin-set",PINCFG_TYPE_SET },
    { "nationalchip,pin-clr",PINCFG_TYPE_CLR },
    { "nationalchip,pin-rev",PINCFG_TYPE_REV },
    { "nationalchip,pin-open",PINCFG_TYPE_OPEN },

    { "nationalchip,pin-pdm0",PINCFG_TYPE_PDM0 },
    { "nationalchip,pin-pdm1",PINCFG_TYPE_PDM1 },
    { "nationalchip,pin_pdm2",PINCFG_TYPE_PDM2 },
    { "nationalchip,pin_pdm3",PINCFG_TYPE_PDM3 },
    { "nationalchip,pin_pdm4",PINCFG_TYPE_PDM4 },
    { "nationalchip,pin_pdm5",PINCFG_TYPE_PDM5 },
    { "nationalchip,pin_pdm6",PINCFG_TYPE_PDM6 },
    { "nationalchip,pin_pdm7",PINCFG_TYPE_PDM7 },
};
		...
		unsigned *num_maps)
{
	...
	ret = of_property_read_u32(np, "nationalchip,pin-dir", &val);
	if (!ret)
		has_func = true;
...
static void nationalchip_gpio_set(struct gpio_chip *gc, unsigned offset, int value)
{
	...
	if (value){
		data = readl(reg + 0x10);
		data |= 1 << offset;
		writel(data, reg + 0x10);
	}
	else{
		data = readl(reg + 0x18);
		data |= 1 << offset;
		writel(data, reg + 0x18);
	}
...
static int nationalchip_gpio_direction_output(struct gpio_chip *gc, unsigned offset,
                            int value)
{
	return nationalchip_gpio_set_direction(gc, offset, true);
}
                ...
                struct nationalchip_pmx_func *func)
{
    ...
    if (of_property_read_u32(func_np, "nationalchip,pin-function", &func->val))
        return 0;
...
static void nationalchip_gpio_irq_handler(struct irq_desc *desc)
{
	...
	for (bank_num = 0; bank_num < drvdata->nr_banks; bank_num++) {
		;
	}
	data = readl(irq_base + 0x24);
	if (data != 0){
		unsigned int gpio_irq;
		gpio_irq = irq_find_mapping(gc->irqdomain, 8);
        generic_handle_irq(gpio_irq);
	}
	chained_irq_exit(chip, desc);
}

void nationalchip_gpio_set_imask(struct nationalchip_pinctrl_drv_data *drvdata,
		struct nationalchip_pin_bank *bank, unsigned int offset, bool enable)
{
	unsigned long flags;
    void __iomem    *irq_base;
	u32 data;
	...
	data = readl(irq_base + 0x7c);
	if (enable)
		data = ~(1 << offset);
	else
		data |= (1 << offset);
	writel(data, irq_base + 0x7c);
...
static int nationalchip_gpio_set_irq_type(struct irq_data *d, unsigned int type)
{
	...
	data = readl(irq_base + 0x28);
	data &= edge_config;
	writel(data, irq_base + 0x28);
...
static void __exit nationalchip_pinctrl_drv_unregister(void)
{
	...
	list_for_each_entry(drvdata, &drvdata_list, node) {
		/*free_irq(drvdata->irq, "test_gpio_irq");*/
...
/*module_init(nationalchip_pinctrl_drv_register);*/
