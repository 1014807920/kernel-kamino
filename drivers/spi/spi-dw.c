...
static void dw_spi_set_cs(struct spi_device *spi, bool enable)
{
	...
	if(dws->cs_reg)
		*(volatile uint32_t*)dws->cs_reg = enable;
...
static irqreturn_t dw_spi_irq(int irq, void *dev_id)
{
	...
	if(dws->is_int_from_mcu){
		data = readl(dws->ifm_reg);
		if (!(data & (1 << dws->ifm_chan)))
			return IRQ_NONE;

		data = 1 << dws->ifm_chan;
		writel(data, dws->ifm_reg + 4);	/* clear ck to arm intc spi bit */
	}
