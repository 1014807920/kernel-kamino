...
int gxasoc_stream_init(struct aout_stream *stream,
		...
		unsigned int (*max_subdev_callback)(void))
{
	...
	aout_dev_set_i2s_enable (stream->optReg, true);
#endif
...
int gxasoc_stream_uninit(struct aout_stream *stream)
{
#if defined(AOUT_RECORD)
...
#endif
#if defined(CONFIG_ARCH_LEO)
	aout_dev_set_i2s_enable(stream->optReg, false);
#endif
	aout_spd_set_enable(stream->optReg, false);
...
int gxasoc_stream_power_suspend(struct aout_stream *stream)
{
	int ret = 0;
...
int gxasoc_stream_power_resume(struct aout_stream *stream)
{
...
static int _post_axi_idle(struct aout_stream *stream)
{
...
int gxasoc_stream_interrupt(struct aout_stream *stream)
{
	...
	if (audioIrqStatus & (0x1 << 20))
		_post_axi_idle(stream);

	return completeFlags;
}

int gxasoc_stream_set_global_volume(struct aout_stream *stream, long int dbValue)
{
	enum aout_subdevice subdev = AOUT_PLAYBACK0;
...
int gxasoc_stream_get_global_volume(struct aout_stream *stream, long int *dbValue)
{
	if (dbValue)
...
int gxasoc_stream_set_global_mute(struct aout_stream *stream, long int mute)
{
	if (mute) {
...
int gxasoc_stream_get_global_track(struct aout_stream *stream, enum aout_track *track)
{
	if (track)
...
int gxasoc_stream_set_global_track(struct aout_stream *stream, enum aout_track track)
{
#if defined (CONFIG_ARCH_LEO_MPW)
	stream->globalTrack = STEREO_TRACK;
#endif
...
int gxasoc_stream_get_global_mute(struct aout_stream *stream, long int *mute)
{
...
int gxasoc_stream_open(struct aout_substream *substream,
...
int gxasoc_stream_close(struct aout_substream *substream)
{
	struct aout_stream *stream = substream->stream;
...
int gxasoc_stream_config(struct aout_substream *substream, struct aout_params *params)
{
	struct aout_stream *stream = substream->stream;
...
int gxasoc_stream_run(struct aout_substream *substream)
{
	struct aout_stream *stream = substream->stream;
...
int gxasoc_stream_rerun(struct aout_substream *substream)
{
...
int gxasoc_stream_stop(struct aout_substream *substream)
{
	return 0;
}

int gxasoc_stream_pause(struct aout_substream *substream)
{
	int ret = 0;
	struct aout_stream *stream = substream->stream;
...
int gxasoc_stream_resume(struct aout_substream *substream)
{
	struct aout_stream *stream = substream->stream;
...
int gxasoc_stream_read(struct aout_substream *substream, unsigned char *buffer, unsigned int size)
{
	printk("=======> %s %d\n", __FUNCTION__, __LINE__);
	return 0;
}

int gxasoc_stream_write(struct aout_substream *substream, unsigned char *buffer, unsigned int size)
{
	struct aout_buffer *buf = &substream->buffer;
...
unsigned int gxasoc_stream_free_space(struct aout_substream *substream)
{
	return _free_space(substream);
}

int gxasoc_stream_set_volume(struct aout_substream *substream, long int dbValue)
{
	struct aout_stream *stream = substream->stream;
...
int gxasoc_stream_get_volume(struct aout_substream *substream, long int *dbValue)
{
...
int gxasoc_stream_set_mute(struct aout_substream *substream, long int  mute)
{
	struct aout_stream *stream = substream->stream;
...
int gxasoc_stream_get_mute(struct aout_substream *substream, long int *mute)
{
...
int gxasoc_stream_set_track(struct aout_substream *substream, enum aout_track track)
{
	struct aout_stream *stream = substream->stream;
...
int gxasoc_stream_get_track(struct aout_substream *substream, enum aout_track *track)
{
...
int gxasoc_stream_volume_range(long int *minDBValue, long int *maxDBValue)
{
	if (minDBValue)
