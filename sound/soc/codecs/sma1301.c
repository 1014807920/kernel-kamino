...
/* SMA1301 I2C Register Set - filename.datx 
   Generated and Saved by the SMA1301 Control Panel V1.02  
   19:00:34, 07-12-2018  */
static unsigned int sma1301_reg_original_value[256] = {
	0x80, 0x00, 0x00, 0x01, 0x17, 0x01, 0x00, 0x01, 	/* 0x00 - 0x07 */
	0x17, 0x4b, 0x33, 0x30, 0x58, 0x58, 0xfe, 0x0f, 	/* 0x08 - 0x0f */
	0x04, 0xe1, 0x00, 0x23, 0x12, 0x01, 0x08, 0x08, 	/* 0x10 - 0x17 */
	0x14, 0x09, 0x27, 0x06, 0x01, 0x0f, 0x0f, 0x0f, 	/* 0x18 - 0x1f */
	0x00, 0x00, 0x00, 0x1e, 0x05, 0x05, 0x41, 0x19, 	/* 0x20 - 0x27 */
	0x00, 0x00, 0x04, 0x07, 0x0c, 0x0c, 0x2c, 0x2c, 	/* 0x28 - 0x2f */
	0x2c, 0x2c, 0x38, 0x00, 0x00, 0x59, 0x93, 0x3f, 	/* 0x30 - 0x37 */
	0x00, 0x1d, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 	/* 0x38 - 0x3f */
	0x38, 0x2c, 0x04, 0x8a, 0x31, 0x01, 0x3f, 0x1a, 	/* 0x40 - 0x47 */
	0x02, 0xc8, 0xb9, 0x02, 0x75, 0xce, 0x07, 0x27, 	/* 0x48 - 0x4f */
	0x35, 0x06, 0xc0, 0x4d, 0x01, 0x35, 0x53, 0x07, 	/* 0x50 - 0x57 */
	0xe3, 0x76, 0x02, 0x3f, 0xb2, 0x07, 0x35, 0x15, 	/* 0x58 - 0x5f */
	0x03, 0x8b, 0xeb, 0x00, 0x40, 0x00, 0x00, 0xca, 	/* 0x60 - 0x67 */
	0xea, 0x05, 0x74, 0x15, 0x00, 0x25, 0xe9, 0x05, 	/* 0x68 - 0x6f */
	0xa0, 0xdb, 0x02, 0x40, 0x00, 0x00, 0xda, 0x16, 	/* 0x70 - 0x77 */
	0x03, 0x5f, 0x24, 0x06, 0x0d, 0x48, 0x00, 0xe1, 	/* 0x78 - 0x7f */
	0x7e, 0x03, 0x40, 0x00, 0x00, 0xf2, 0xb8, 0x00, 	/* 0x80 - 0x87 */
	0x1e, 0x81, 0x05, 0xe7, 0x70, 0x00, 0x00, 0x0a, 	/* 0x88 - 0x8f */
	0x00, 0xf2, 0x40, 0x02, 0x05, 0x08, 0xf3, 0x7e, 	/* 0x90 - 0x97 */
	0xdf, 0xff, 0xff, 0xfd, 0xd7, 0xdb, 0xf9, 0x37, 	/* 0x98 - 0x9f */
	0xf3, 0xff, 0x00, 0x28, 0x40, 0xff, 0xf8, 0xea, 	/* 0xa0 - 0xa7 */
	0xc8, 0xff, 0x7d, 0x3d, 0x60, 0xff, 0xe9, 0x6f, 	/* 0xa8 - 0xaf */
	0xf3, 0xff, 0xea, 0x6a, 0x73, 0xff, 0xfa, 0x7e, 	/* 0xb0 - 0xb7 */
	0xff, 0xff, 0xff, 0xff, 0xdf, 0xff, 0xf9, 0xff, 	/* 0xb8 - 0xbf */
	...
	0x00, 0x00, 0xc0, 0x00, 0x3f, 0x00, 0x00, 0x0a, 	/* 0xf8 - 0xff */
	
};

static struct reg_default sma1301_reg_def[256];
...
static void init_sma1301_reg_def_data(void)
{
	unsigned int index;
	for(index = 0x00; index <= SMA1301_FF_DEVICE_INDEX; index++){
		sma1301_reg_def[index].reg = index;
		sma1301_reg_def[index].def = sma1301_reg_original_value[index];
	}
}
...
static int sma1301_init_codec_reg(struct snd_soc_codec *codec)
{
	struct sma1301_priv *sma1301 = snd_soc_codec_get_drvdata(codec);
	...
	for (i = 0; i <= SMA1301_FF_DEVICE_INDEX; i ++) {
		//if(!sma1301_writeable_register(codec->dev, i))
		//	continue;
		regmap_write(sma1301->regmap, sma1301_reg_def[i].reg, sma1301_reg_def[i].def);
	}
...
void sma1301_hp_detect(struct sma1301_priv *sma1301)
{
	int gpio_val = 0;
	...
	if (gpio_val != sma1301->gpio_val) {
		if (gpio_val) {									//hp in
			...
			regmap_update_bits(sma1301->regmap, SMA1301_10_SYSTEM_CTRL1, SPK_MODE_MASK, SPK_OFF);
			if(!is_mute_setted)
			...
			if(!is_mute_setted){
				regmap_update_bits(sma1301->regmap, SMA1301_10_SYSTEM_CTRL1, SPK_MODE_MASK, SPK_MONO);
			}
		}
...
static void sma1301_delayed_work(struct work_struct *data)
{
	struct sma1301_priv *sma1301 = container_of(data, struct sma1301_priv, detect_work.work);
	sma1301_hp_detect(sma1301);
	schedule_delayed_work(&sma1301->detect_work, msecs_to_jiffies(20));
	return;
}
...
static int sma1301_probe(struct snd_soc_codec *codec)
{
	...
	//regcache_cache_bypass(sma1301->regmap, true);
	//sma1301_reset(codec);
	sma1301_init_codec_reg(codec);
	...
	if (sma1301->need_hp_det) {
		...
		INIT_DELAYED_WORK(&sma1301->detect_work, sma1301_delayed_work);
		schedule_delayed_work(&sma1301->detect_work, msecs_to_jiffies(20));
	}
				...
				const struct i2c_device_id *id)
{
	...
	ret = regmap_read(sma1301->regmap, SMA1301_FF_DEVICE_INDEX, &value_id);
	if (ret < 0) {
		dev_err(&client->dev, "Failed to read device id from the sma1301: %d\n", ret);
		return ret;
	}
	if ((value_id != 0x09) && (value_id != 0x0a)) {
		dev_err(&client->dev, "Not a sma1301 chip =0x%x\n", value_id);
		return -ENODEV;
	}
 ...
 //		sma1301->need_hp_det = false;
	...
	if (np) {
		...
		if (sma1301->reg_array == NULL)
			dev_info(&client->dev, "There is no registers-of-amp from DT\n");
	}
